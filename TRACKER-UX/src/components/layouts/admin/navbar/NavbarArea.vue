...
<template>
  ...
  >
    ...
    <div>
      <p v-if="authStore.user" class="text-grass12 font-bold uppercase">
        {{ authStore.user.username }}
        <Badge v-if="authStore.user?.personalDetails">{{ authStore.user?.personalDetails?.position }}</Badge>
      </p>
    ...
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" class="bg-white">
        ...
        <DropdownMenuItem>
          <router-link
            :to="{ name: 'settings' }"
            class="ml-2 leading-none outline-none flex items-center justify-end"
          >
            Settings
          </router-link>
        </DropdownMenuItem>
        ...
        <DropdownMenuItem>
          <DropdownMenuItem>
            <AlertDialogRoot>
              <AlertDialogTrigger
                @click.stop
                class="text-red11 font-semibold leading-none outline-none flex items-center justify-end"
                >Logout
              </AlertDialogTrigger>
              <AlertDialogPortal>
                <AlertDialogOverlay
